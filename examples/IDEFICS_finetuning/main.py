...
class LogScoringModuleFn_idefics(BaseModuleFunction):
    ...
    def forward(self, forward_outputs, minibatch, action_lengths, **kwargs):
        assert self._model_type == "idefics"

        shift_logits = forward_outputs['logits'][..., :-1, :].contiguous()
        shift_labels = minibatch["input_ids"][..., 1:].contiguous()


        tokens_logprobs = \
            torch.gather(shift_logits, 2, shift_labels[:, :, None]).squeeze(-1).to(torch.float32)

        mask = torch.ones(shift_labels.shape, dtype=torch.bool, device=self.device)
        for _b, _a_len in zip(mask, action_lengths):  # example [84], 3
            _b[-_a_len:] = torch.zeros((_a_len), device=self.device)
        
        masked_token_probs = tokens_logprobs.masked_fill(mask, 0.0) 
        minibatch_probs = masked_token_probs.sum(-1)

        return minibatch_probs.cpu()

class ValueHeadModuleFn_idefics(BaseModuleFunction):
    def __init__(self, model_type, pre_encoded_input):
        super().__init__()
        self._model_type = model_type
        self._pre_encoded_input = pre_encoded_input
    ...
    def forward(self, forward_outputs, minibatch, action_lengths, **kwargs):
        ...
        model_head = forward_outputs['hidden_states'][-1][row_indexes, actions_to_last_context_token_index, :]
...
class PeftInitializer(BaseModelInitializer):
    ...
    def _get_model_config(self):
        ...
        elif "idefics" in self._model_name:
            return LoraConfig(
                r=self._r,
                lora_alpha=self._alpha,
    ...
    def initialize_model(self, model):
        if self._use_lora:
            if self._model_type != 'idefics':   
                llm_module = model._modules['_LLM_model']
            else:
                llm_module = model._modules['_VLM_model']
            ...
            # TODO CHECK WHY IDEFICS GIVES NANs WHEN USING prepare_model_for_kbit_training
            #if self._use_4bit:
            ...
            if self._model_type != 'idefics':
                ...
                model._modules['_VLM_model'] = peft_model

        model.eval()  # Important to ensure ratios are 1 in first minibatch of PPO (i.e. no dropout)
        if self._model_type == 'idefics':
            model._modules['_VLM_model'].config.use_cache = self._use_cache
        else:
...
def main(config_args):
    ...
    if config_args.lamorel_args.llm_args.model_type == 'idefics':
        custom_module_functions={
        'score': LogScoringModuleFn_idefics(config_args.lamorel_args.llm_args.model_type,
                                            config_args.lamorel_args.llm_args.pre_encode_inputs),
        'value': ValueHeadModuleFn_idefics(config_args.lamorel_args.llm_args.model_type,
                                   config_args.lamorel_args.llm_args.pre_encode_inputs)}
    else:
        custom_module_functions={
    ...
    lm_server = Caller(config_args.lamorel_args,
                       ...
                       custom_module_functions=custom_module_functions)
